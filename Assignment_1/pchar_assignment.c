...
static int devcnt = 3; // Default value
...
static __init int pchar_init(void) {
    ...
    if (!devices) {
        ret = -ENOMEM;
        printk(KERN_ERR "%s: kmalloc_array() failed to allocate devices private struct memory.\n", THIS_MODULE->name);
        goto devices_kmalloc_failed;
    }
    printk(KERN_INFO "%s: kmalloc_array() allocated devices private struct memory.\n", THIS_MODULE->name);
